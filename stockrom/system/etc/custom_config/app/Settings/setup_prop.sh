...
sed -i 's/^ro.config.ringtone=.*$/ro.config.ringtone=Light_your_smart_world.ogg/g' $SYSTEM_PROPERTY_PATH_DESTINATION
#sed -i 's/^ro.config.notification_sound=.*$/ro.config.notification_sound=F1_New_SMS.ogg/g' $SYSTEM_PROPERTY_PATH_DESTINATION
#sed -i 's/^persist.sys.timezone=.*$/persist.sys.timezone=Europe\/Amsterdam/g' $SYSTEM_PROPERTY_PATH_DESTINATION
